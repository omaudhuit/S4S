...
# kWh Estimation based on Electricity Cost (Εύρεση kWh βάσει €)
...
# Corporate Consumers (Εταιρικοί Καταναλωτές Χ.Τ)
def compute_corporate():
    ...
    # --- Intermediate Calculations ---
    # I7 for corporate: different percentages based on time-of-use
    ...
    # I11: as in the residential case
    if region in ["Νότιο Αιγαίο", "Κρήτη"]:
        I11 = 1600
    elif region in ["Αττική", "Δυτική Ελλάδα", "Ιόνιοι Νήσοι"]:
        I11 = 1550
    ...
    # I10: based on roof type and orientation
    if roof_type == "Στέγη":
        if orientation in ["Α", "Δ"]:
            I10 = 0.85
        elif orientation in ["ΝΑ", "ΝΔ"]:
    ...
    # L7: Inverse calculation from annual consumption
    L7 = annual_consumption / I12

    # O7: Effective tariff rate
    if tariff_category == "Γ1Ν":
        O7 = normal_tariff * I7 + night_tariff * I8
    else:
        O7 = normal_tariff
    ...
    # For corporate consumers, we estimate the baseline cost similar to the residential case
    if tariff_category == "Γ1":
        L17 = (normal_tariff + 0.046) * 1.06 * annual_consumption
    else:
        L17 = None
...
# Main routine to run all calculations and print results
def main():
    print("=== Residential Consumers Data ===")
    residential_results = compute_residential()
    for key, value in residential_results.items():
        print(f"{key}: {value}")

    print("\n=== Loan Calculation Data ===")
    loan_results = compute_loan()
    for key, value in loan_results.items():
        print(f"{key}: {value}")

    print("\n=== kWh Estimation Data ===")
    kwh_results = compute_kwh_estimation()
    for key, value in kwh_results.items():
        print(f"{key}: {value}")

    print("\n=== Corporate Consumers Data ===")
    corporate_results = compute_corporate()
